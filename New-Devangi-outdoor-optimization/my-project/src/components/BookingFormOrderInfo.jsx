  ...
  return (
    <div className="relative w-full">
      ...
      <input
        ...
        className={`border ${
          error ? "border-red-300" : "border-gray-200"
        } px-4 py-2 rounded-xl w-full bg-white text-[var(--color-text)] focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-200 shadow-sm hover:shadow-md h-10`}
      />
      {error && (
        <p className="absolute -bottom-5 left-0 text-red-500 text-xs mt-1 animate-slideDown flex items-center gap-1">
          <FiAlertCircle className="inline-block" /> {error}
...
// --- Stepper Component with Code 2 Styling ---
...
].map(opt => ({ ...opt, mandatory: 'true' })); // Ensure CustomSelect renders * for required field
...
export default function BookingFormOrderInfo() {
  ...
  // Validation state from Code 2
  const [errors, setErrors] = useState({});

  // 1. Fetch Spaces (Logic from Code 1, including the fix for transit fields)
  useEffect(() => {
    const fetchSpaces = async () => {
      try {
        ...
        .map(space => ({
          ...
          transitLine: space.transitLine, // FIX: Added transit fields
        }));
        setSpaces(transformed);
        setLoading(false);
      } catch (error) {
        console.error("Failed to load spaces:", error);
        toast.error("Failed to load advertising space list.");
        setLoading(false);
      }
    };
    fetchSpaces();
  ...
  // 2. Core Logic (Adapted from Code 1 & 2)
  ...
  // Validation function (Copied from Code 2)
  const validateCampaign = (campaign) => {
    const newErrors = {};

    if (!campaign.campaignName || campaign.campaignName.trim().length < 3) {
      newErrors.campaignName = 'Campaign name is required and must be at least 3 characters.';
  ...
  const handleCampaignChange = (index, e) => {
    ...
    if (name === 'isFOC') {
      // Logic for radio buttons: 'true'/'false' string to boolean
  ...
  const addCampaign = () => {
    ...
    // Add an empty error object for the new campaign
    setErrors((prev) => ({ ...prev, [orderInfo.campaigns?.length || 0]: {} }));
  };
  ...
  const deleteCampaign = (index) => {
    ...
    // Remove errors for the deleted index
    setErrors((prev) => {
      const newErrors = {};
  ...
  const saveCampaign = (index) => {
    ...
    if (Object.keys(validationErrors).length > 0) {
      setErrors((prev) => ({ ...prev, [index]: validationErrors }));
      toast.error('Please fix validation errors before saving. ⚠️');
      return;
    }
    ...
    toast.success(`Campaign "${campaign.campaignName}" saved successfully!`);
  };
  ...
  const handleNext = () => {
    if (!orderInfo.campaigns || orderInfo.campaigns.length === 0) {
      toast.error('Please add at least one campaign to continue. ➕');
      return;
    }

    const hasUnsavedCampaigns = orderInfo.campaigns.some((c, index) => !c.isSaved || Object.keys(errors[index] || {}).length > 0);
    if (hasUnsavedCampaigns) {
      // Trigger validation for all unsaved/errored campaigns
      let allPassed = true;
      const newErrors = {};

      orderInfo.campaigns.forEach((campaign, index) => {
        if (!campaign.isSaved) {
          const validationErrors = validateCampaign(campaign);
          if (Object.keys(validationErrors).length > 0) {
      ...
      if (!allPassed) {
        toast.error('Please save all campaigns and fix all validation errors before proceeding. 🛑');
        return;
      }
    }
  ...
  // 3. Render UI (Code 2 styling applied)
  return (
          ...
          {loading ? (
            ...
            <>
              {orderInfo.campaigns?.map((campaign, index) => (
                <Card key={index} className="relative mb-8">
                  <CardContent>
                    {campaign.isSaved ? (
                      // Display Saved Campaign Info
                      <div className="flex justify-between items-start">
                        <div>
                          <h3 className="text-xl font-semibold mb-1">{campaign.campaignName}</h3>
                          <p className="text-sm text-gray-600">Industry: {campaign.industry}</p>
                          <p className="text-sm text-gray-600">
                            From **{campaign.startDate}** to **{campaign.endDate}**
                          </p>
                          {campaign.description && <p className="text-sm italic mt-2">Description: {campaign.description}</p>}
                          {campaign.isFOC && (
                          ...
                          <p className="text-sm mt-2">
                             **{campaign.selectedSpaces.length} spaces** selected.
                          </p>
                        </div>
                      ...
                      // Edit/New Campaign Form
                      <>
                        <h2 className="text-xl font-bold mb-6 text-gray-800">Campaign #{index + 1} Details</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-10 mb-8">
                          <Input
                            ...
                            onBlur={(e) => {
                                const validationErrors = validateCampaign(orderInfo.campaigns[index]);
                                setErrors((prev) => ({ ...prev, [index]: validationErrors }));
                            }}
                          />
                          <CustomSelect
                          ...
                          <div className="col-span-full flex flex-col space-y-1">
                            ...
                            <textarea
                              ...
                              maxLength={400}
                              placeholder="Optional description for the campaign..."
                            />
                        ...
                        {/* Inventory Selector */}
                        <div className='mt-4'>
                          <h3 className="text-lg font-semibold mb-4 text-gray-800">Select Spaces for Campaign</h3>
                          <InventorySelector
                            campaignIndex={index}
                            campaign={campaign}
                            spaces={spaces}
                        ...
                        {/* Action Buttons */}
                        <div className="flex mt-8 justify-between pt-4 border-t border-gray-200">
                          <Button
                            onClick={() => deleteCampaign(index)}
                            className="bg-red-100 text-red-700 hover:bg-red-200"
                          >
              ...
              <Button 
                onClick={addCampaign} 
                className="bg-[black] text-white hover:bg-gray-800 transition-all duration-200 transform hover:scale-105 active:scale-95 shadow-md hover:shadow-lg mt-4"
              >
                + Add New Campaign
              </Button>
            </>
