...
const PreviewField = ({ label, value }) => (
  <div className="w-full">
    ...
    {/* Use the styling from Code 2 */}
    <p className="border px-4 py-2 rounded-xl w-full bg-white text-[var(--color-text)] shadow-sm whitespace-pre-wrap">
      {value || "-"}
    </p>
  </div>
);
...
export default function BookingPreview() {
  ...
  const computeTotalPrice = () => {
    // Logic from Code 2 to account for isFOC in total price calculation
  ...
  const handleSubmitBooking = async () => {
    ...
    try {
      ...
      // Payload structure from Code 1 (including isFOC)
      const campaignsPayload = orderInfo.campaigns.map((c) => ({
        ...
        endDate: c.endDate,
        isFOC: c.isFOC, // <-- Important field from Code 1
      ...
      // console.log("FRONTEND IS SENDING THIS PAYLOAD:", JSON.stringify(campaignsPayload, null, 2)); // Retaining log for reference
      ...
      // Updated navigation from Code 2 for better UX
  ...
  const handleSaveProposal = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        // Note: Dates (startDate, endDate) and isFOC are intentionally excluded from the proposal payload in Code 1's logic
      ...
      // console.log("Saving Proposal with this FLAT payload (NO DATES):", JSON.stringify(payload, null, 2)); // Retaining log for reference
    ...
    <div className={`min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 w-screen text-[var(--color-text)] flex flex-col lg:flex-row overflow-hidden`}>
      ...
      <main className={`flex-1 overflow-y-auto px-4 md:px-6 py-8 transition-all duration-300 ${isCollapsed ? 'lg:ml-24' : 'lg:ml-64'}`}>
        <div className="max-w-screen-xl w-full mx-auto">
                ...
                {Object.entries(basicInfo)
                  .filter(
                    ([key]) =>
                      ...
                      key !== "user" &&
                      key !== "campaignImages" &&
                      key !== "agencyName" // Agency Name is conditional
                  )
                  .map(([key, value]) => (
                    <PreviewField
                      key={key}
              ...
              {orderInfo.campaigns?.filter((c) => c.isSaved).map((campaign, cIdx) => (
                <div key={cIdx} className="mb-6 border-b pb-6 last:border-b-0 last:pb-0">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                    ...
                    {/* Display start and end dates together as a duration field, as styled in Code 2 */}
                  ...
                  <div className="overflow-x-auto rounded-xl border border-gray-200">
                    <table className="min-w-full text-sm">
                      <thead className="bg-gray-100">
                        <tr>
                          {/* Columns from Code 2's table structure (more concise) */}
                      ...
                      <tbody>
                        {campaign.selectedSpaces.map((space) => (
                          <tr key={space.id} className="border-t border-gray-200">
                            <td className="px-4 py-3 text-left text-purple-700">
                              {space.name}
                            </td>
                            ...
                            <td className="px-4 py-3 text-right font-semibold">
                              {/* Display total price for this space/units, using Code 2's formatting */}
          ...
          {/* Total Price Card */}
          <Card className="mb-24 animate-slideIn">
            <CardContent className="flex justify-between items-center">
              <h2 className="text-xl font-bold text-gray-800">Total Price</h2>
              <span className="text-2xl font-bold text-orange-500">
                {/* Use the computed total price and Code 2's formatting */}
