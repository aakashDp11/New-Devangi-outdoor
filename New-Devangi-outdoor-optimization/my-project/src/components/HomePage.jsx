  ...
  // );
  const Card = ({ children, className = '', ...props }) => (
    <div className={`bg-white border border-gray-200 shadow-sm rounded-xl w-full h-full flex flex-col ${className}`} {...props}>
      {children}
    </div>
  );
  ...
  console.log("Space res is",spaceRes);
        const firstPageRes = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/bookings/optimized?page=1&limit=50`, { headers });
          const bookingDataRes= await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/bookings/countDetails`, { headers });
          if (bookingDataRes.ok) {
            const bookingData = await bookingDataRes.json(); // Parse the JSON response
            console.log("New book data is", bookingData); 
            setBookings(bookingData.bookings);
        } else {
            console.error("Error fetching bookings:", bookingDataRes.statusText);
        }
        if (!firstPageRes.ok) {
        ...
        console.log("All bookins are",allBookings);
        setBookingStats(bookingData.bookingStats || []);
        console.log("Stats booking are",bookingData.bookingStats);
        // setBookings(bookingData.bookingStats || []);
  ...
  // console.log("Booking map is",bookingsData);
  console.log("Proposals are",proposals);
      ...
      console.log("Booking data is",proposalsData);
    ...
    const pipelineBarData = {
      labels: ['Booking Confirmed', 'Artwork', 'PO', 'Invoice'],
      values: [
