...
//       console.log("Digital Status (ds):", ds);
  
//       const currentCampaignId = String(pipelineData?.campaign?._id || '');
//       console.log("Current Campaign ID:", currentCampaignId);
  
//       if (ds && typeof ds === 'object') {
//         console.log("Checking if ds.confirmed and campaignId match:", ds.confirmed, ds.campaignId, currentCampaignId);
...
//       const ds = Array.isArray(campaignDigital) ? campaignDigital.find(x => x.unitId === 1) : null;
//       console.log("Live Status (ds):", ds);
  
//       const currentCampaignId = String(pipelineData?.campaign?._id || '');
//       console.log("Current Campaign ID:", currentCampaignId);
  
//       if (ds && typeof ds === 'object') {
//         console.log("Checking if ds.isLive and campaignId match:", ds.isLive, ds.campaignId, currentCampaignId);
...
//   // Ensure campaignDigital is set and is an array
//   if (Array.isArray(campaignDigital)) {
//     const ds = campaignDigital.find(x => x.unitId === 1);  // Find the digital status for unitId 1
...
//                 timestamp={formatTimestamp(ds.confirmed ? ds.completedAt : null)}  // Show completedAt timestamp if confirmed
//               />
//             )
//           },
...
//               <NodeLabel
//                 title="Is Live"
//                 timestamp={formatTimestamp(ds.isLive ? ds.liveCompletedAt : null)}  // Show liveCompletedAt timestamp if isLive
//               />
//             )
//           },
...
//       // Edge from Digital → Live if unit is confirmed (campaign-scoped)
//       if (ds.confirmed) {
//         dynamicEdges.push({
...
//     // Digital status per space
//     if (id.startsWith('digital-')) {
//       // campaign-scoped digital status per space
//       return campaignDigital?.[spaceId] || null;  // Pass the entire array for the digital status
//     }
...
function CampaignPipelineInternal({ campaignId, isFOC }) {
  ...
  // Inside isNodeCompleted function:
// const isNodeCompleted = (nodeId, space = null) => {
//   // ... (existing booking, po, artwork, invoice, payment logic)

//   // Handle unit-specific printing/mounting/digital/live nodes
//   if (nodeId.startsWith('print-') || nodeId.startsWith('mount-') || nodeId.startsWith('digital-') || nodeId.startsWith('live-')) {
...
//     }
//     if (nodeId.startsWith('digital-')) {
//         // Use the campaignDigital structure for digital status
//         const digitalStatusesForSpace = campaignDigital[spaceId] || [];
//         const ds = digitalStatusesForSpace.find(x => x.unitId === unitId);
//         // Also ensure campaignId matches if available on the digital status object
//         return ds?.confirmed && String(ds.campaignId || '') === String(pipelineData?.campaign?._id || '');
//     }
//     if (nodeId.startsWith('live-')) {
//         // Use the campaignDigital structure for live status
//         const digitalStatusesForSpace = campaignDigital[spaceId] || [];
//         const ds = digitalStatusesForSpace.find(x => x.unitId === unitId);
//         // Also ensure campaignId matches if available on the digital status object
//         return ds?.isLive && String(ds.campaignId || '') === String(pipelineData?.campaign?._id || '');
//     }
//   }
...
//     if (space.spaceType === 'DOOH') {
//       // ... (existing DOOH logic remains largely the same, maybe adjust positioning)
//       // For DOOH, campaignDigital is already an array of digital statuses.
//       // We should iterate campaignDigital[space._id] if it's an array
//       // This part might need further refinement based on how you want to visualize multiple digital units if they exist.
//       // For now, assuming campaignDigital[spaceId] holds an array of digital statuses for units.
//       const digitalStatusesForSpace = campaignDigital[space._id] || [];

//       digitalStatusesForSpace.forEach((ds, unitIndex) => {
//           const digitalId = `digital-${space._id}-${ds.unitId}`; // Unique ID for digital node per unit
//           const liveId = `live-${space._id}-${ds.unitId}`; // Unique ID for live node per unit

//           // Adjust Y position for each unit's branch
...
//                       label: (
//                           <NodeLabel
//                               title={`Digital Agency (Unit ${ds.unitId})`}
//                               timestamp={formatTimestamp(ds.confirmed ? ds.completedAt : null)}
//                           />
//                       )
//                   },
//                   position: { x: 850, y: unitBranchY },
//                   style: getNodeStyle(isNodeCompleted(digitalId, space)), // Pass space for context
//               },
//               {
//                   id: liveId,
//                   data: {
//                       label: (
//                           <NodeLabel
//                               title={`Is Live (Unit ${ds.unitId})`}
//                               timestamp={formatTimestamp(ds.isLive ? ds.liveCompletedAt : null)}
//                           />
//                       )
//                   },
//                   position: { x: 1050, y: unitBranchY },
//                   style: getNodeStyle(isNodeCompleted(liveId, space)), // Pass space for context
//               }
//           );

//           dynamicEdges.push({
//               id: `e-${inventoryId}-digital-${ds.unitId}`,
//               source: inventoryId,
//               target: digitalId,
//               markerEnd: 'arrowclosed'
//           });
...
//           if (ds.confirmed) {
//               dynamicEdges.push({
//                   id: `e-digital-${ds.unitId}-live-${ds.unitId}`,
//                   source: digitalId,
//                   target: liveId,
//                   markerEnd: 'arrowclosed'
//               });
...
//     } else { // For 'Billboard' or other non-DOOH types
...
//           {
//             id: printNodeId,
//             data: { label: <NodeLabel title={`Printing Status (Unit ${unitId})`} timestamp={formatTimestamp(printStatus.completedAt)} /> },
//             position: { x: 850, y: unitBranchY },
//             style: getNodeStyle(printStatus.confirmed), // Use specific unit's confirmed status
//           },
//           {
//             id: mountNodeId,
//             data: { label: <NodeLabel title={`Mounting Status (Unit ${unitId})`} timestamp={formatTimestamp(mountStatus?.completedAt)} /> },
//             position: { x: 1050, y: unitBranchY },
