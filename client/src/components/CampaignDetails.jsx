...
import { toast } from "sonner"; // Using 'sonner' as per Code 1 logic
...
const Card = ({ children, className = "", ...props }) => (
  <div
    className={`
      bg-white shadow-xl rounded-2xl w-full flex flex-col relative overflow-hidden
      ${className}
    `}
    {...props}
  >
    {/* Optional background flow effect: adjust or remove if too distracting */}
...
// Button component with consistent styling and loading state (from Code 2)
const Button = ({
  children,
  className = "",
  disabled = false,
...
// Input component with a more polished look (from Code 2)
const Input = ({ className = "", error = null, ...props }) => (
  <div className="relative">
    <input
      className={`border ${
        error ? "border-red-300" : "border-gray-200"
      } px-4 py-2 rounded-xl w-full bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all duration-200 shadow-sm hover:shadow-md ${className}`}
      {...props}
    />
    {error && (
...
// Notification component (from Code 2 - Note: Code 1 uses 'sonner', this is a fallback/extra UI)
...
// KeyValueItem component (combined from Code 1 structure and Code 2 styling intention)
const KeyValueItem = ({
  label,
  value,
  icon,
...
// DetailItem component (re-styled from Code 1, using cleaner Code 2-like typography)
const DetailItem = ({ icon, label, value }) => (
  <div className="flex items-center text-xs text-gray-600">
    <span className="w-5 text-blue-500">{icon}</span>
    <span className="font-semibold mr-1 text-gray-700">{label}:</span>
    <span className="text-gray-600">{value}</span>
  </div>
);

// CostInput component (from Code 1 logic, using Code 2's Input component)
const CostInput = ({
  label,
  field,
  value,
  ...
  <div>
    ...
    <div className="relative">
      ...
      <Input
        type={type}
        className={`${isCurrency ? "pl-7" : "px-4"} py-1 text-sm ${
...
export default function CampaignDetails() {
  ...
  const [notifications, setNotifications] = useState([]); // Added for Code 2's Notification UI

  // Functionality to display custom notifications (from Code 2 style)
  ...
  // --- Data Fetching Logic (from Code 1, cleaned up) ---
    ...
    try {
      ...
      const fetchedSpaces = await Promise.all(
        (data.spaces || []).map(async (space) => {
          ...
          console.log("Response data is", res); // Keep Code 1 log
          if (!res.ok) {
      ...
      toast.error("Failed to load campaign details"); // Use toast as in Code 1
    }
  };
  ...
  const fetchPipelineData = async () => {
    try {
      ...
      console.log("Create pipeline started"); // Keep Code 1 log
      ...
      console.log("Pipeline creation data is", res); // Keep Code 1 log
      if (!res.ok) {
        if (res.status === 404) {
          // Fallback data for 404 as in Code 1
          setPipelineData({
      ...
      setPipelineError(true);
      // Fallback data for general error as in Code 1
      setPipelineData({
        artwork: { confirmed: false },
        bookingStatus: { confirmed: false },
        printingStatus: { confirmed: false },
  ...
  // --- Cost Update and Save Logic (from Code 1) ---
  ...
  const handleEditToggle = (spaceId) => {
    ...
    if (updated.has(spaceId)) {
      ...
      // Initialization logic (including invoiceNo) from Code 1
      setEditedCosts((prev) => ({
        ...prev,
        [spaceId]: {
          id: spaceId,
  ...
  const handleSaveCostForSpace = async (spaceId) => {
    ...
    if (!costToSave) {
      toast.error("No changes to save"); // Use toast as in Code 1
      return;
    }

    // Sanitization logic from Code 1
    ...
    try {
      ...
      toast.success("Costs saved for this space!"); // Use toast as in Code 1
      ...
      console.error(err);
      toast.error("Error saving costs for this space."); // Use toast as in Code 1
    }
  };

  if (!campaignData) {
    // Loading UI from Code 2
    return (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 w-screen text-gray-800 flex flex-col lg:flex-row overflow-hidden">
      ...
      {/* Notification System (from Code 2) - Note: Code 1 used 'sonner' */}
        ...
        <div className="flex space-x-4 mt-4 mb-6">
          {["Details", "Pipeline"].map((tab) => (
            <button
              ...
              className={`px-4 py-1 rounded-xl font-medium transition duration-200 shadow-md ${
                activeTab === tab
                  ? "bg-black text-white"
          ...
          {/* Pipeline Error Indicator from Code 1 */}
          {pipelineError && (
        ...
        {activeTab === "Details" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-slideUp">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 flex-grow">
                        ...
                        (step) =>
                          !step.data?.confirmed && (
                            <Card
                              ...
                              className="w-full sm:w-48 cursor-pointer hover:shadow-lg transition-shadow duration-300"
                            >
                  ...
                  {pipelineData?.payment ? (
                    <div>
                      ...
                      <div className="w-[200px] flex flex-col items-center">
                                ...
                                },
                                {
                                  ...
                                  value:
                                    pipelineData.payment.paymentDue === 0
                                      ? 100 // Display full circle if paid up
                                      : pipelineData.payment.paymentDue || 100, // Fallback value
                                  label: "Due",
                                  color: '#F44336', // Red for Due
                                },
                              ],
                              innerRadius: 50,
                              outerRadius: 90,
                              arcLabel: (item) => `${(item.value / (pipelineData.payment.finalAmountWithGST || 1)) * 100}%`,
                        ...
                        {pipelineData?.payment?.payments?.length > 0 ? (
                          <div className="text-xs mt-4 space-y-1 w-full text-center">
                            <p>
                              <strong>Total:</strong> ₹
                              {pipelineData.payment.finalAmountWithGST?.toFixed(2) ?? 0}
                            </p>
                            <p>
                              <strong>Paid:</strong> ₹
                              {pipelineData.payment.totalPaid?.toFixed(2) ?? 0}
                            </p>
                            <p>
                              <strong>Due:</strong> ₹
                              {pipelineData.payment.paymentDue?.toFixed(2) ?? 0}
                            </p>
                          </div>
                        ) : (
                    ...
                    <div className="mt-4">
                      ...
                      <div className="w-[200px] flex flex-col items-center">
                        <div className="rounded-full bg-gray-300 h-[150px] w-[150px] mb-4 animate-pulse"></div>
            ...
            {spaceDetails.map((space, index) => {
              ...
              const printingCost = Number(
                currentCost?.printingcostpersquareFeet || 0
              );
              const mountingCost = Number(
                currentCost?.mountingcostpersquareFeet || 0
              );
              const area = Number(currentCost?.area || computedArea || 0);

              // Total Cost Calculation Logic (from Code 1)
              ...
              switch (space.spaceType) {
                ...
                case "DOOH":
                  // For DOOH, total cost is just the display cost.
                  break;
              ...
              console.log("Current cost is", currentCost); // Keep Code 1 log
                      ...
                      <div className="md:col-span-2">
                        {space.mainPhoto && (
                          <img
                            src={space.mainPhoto}
                            alt={space.spaceName}
                            className="w-full h-32 object-cover rounded-lg border border-gray-200 mb-3 shadow-sm"
                          />
                        )}
                        ...
                        {space.ownershipType === "Traded" && (
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            ...
                            <CostInput
                              ...
                              isDisabled={isFOC}
                            />
                            {/* Invoice No Input Field (from Code 1) */}
                    ...
                    <div className="flex w-full gap-2 mt-4 pt-4 border-t">
                      ...
                      {isEditable && (
                        <Button
                          ...
                          disabled={!isEditable}
                          className="inline-flex items-center gap-2 text-xs ml-auto px-3 py-1.5 bg-blue-600 text-white font-semibold hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                        >
                          <FaSave />
                          Save
                        </Button>
