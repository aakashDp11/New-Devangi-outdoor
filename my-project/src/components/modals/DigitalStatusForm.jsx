...
//   onConfirm,
//   onClose,
//   existingData       // array of { unitId, confirmed, isLive, ... } for this space in this campaign
// }) {
...
//   // Pick the record for the currently selected unit
//   const currentUnitData = useMemo(() => {
//     if (Array.isArray(existingData)) {
//       return existingData.find(d => Number(d?.unitId) === Number(unitId));
//     }
//     if (existingData && typeof existingData === 'object' && 'unitId' in existingData) {
//       return Number(existingData.unitId) === Number(unitId) ? existingData : undefined;
//     }
//     return undefined;
//   }, [existingData, unitId]);
//    useEffect(() => {
...
//            setAvailability({
//              totalUnits: Math.max(1, Number(data?.totalUnits || unit || 1)),
//              free: Array.isArray(data?.free) ? data.free : [],
//              taken: Array.isArray(data?.taken) ? data.taken : []
//            });
//            // Default to first free unit if this campaign has no data for the currently selected unit
//            if (!currentUnitData) {
//              const firstFree = (data?.free || [])[0];
//              if (firstFree) setUnitId(firstFree);
//            }
//          } catch (e) {
//            // fallback: just assume all units are selectable up to 'unit'
...
//   // Hydrate form from selected unit record
//   useEffect(() => {
...
//     // If already confirmed, default to summary; you can still "Edit"
...
//           // prevent saving into a unit already taken by another overlapping campaign
...
//       confirmed: !!confirmed,
//       isLive: !!isLive,
//       goLiveDate,            // keep as 'YYYY-MM-DD' string to match your schema
//       assignedPerson,
//       assignedAgency,
//       note
//     };
...
//             className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
//           >
//             {Array.from({ length: Math.max(1, Number(unit || 1)) }, (_, i) => i + 1).map(n => (
...
//   // Prevent double auto-pick (availability vs existingData)
...
//         ? existingData.map(d => Number(d?.unitId))
//         : (existingData && typeof existingData === 'object' && 'unitId' in existingData)
//           ? [Number(existingData.unitId)]
//           : []
//     );
...
//   // 1) Prefer the campaign’s saved units when existingData arrives
//   useEffect(() => {
//     if (pickedUnitRef.current) return;

//     if (Array.isArray(existingData) && existingData.length > 0) {
//       const firstWithData = [...existingData]
...
//       }
//     } else if (existingData && typeof existingData === 'object' && 'unitId' in existingData) {
//       const id = Number(existingData.unitId);
...
//   // 2) Availability fetch; only pick "first free" if we didn't pick from existingData
//   useEffect(() => {
//     const run = async () => {
//       try {
//         setLoadingAvail(true);
...
//           const hasAnyExisting = Array.isArray(existingData)
...
//   // 3) Hydrate form from selected unit record
//   useEffect(() => {
//     const d = currentUnitData || {};
//     setConfirmed(!!d.confirmed);
//     setIsLive(!!d.isLive);
...
//     // If already confirmed, default to summary; can still "Edit"
//     setView(d.confirmed ? 'summary' : 'form');
//   }, [currentUnitData]);
