...
console.log("orderInfo",orderInfo);
  ...
  const handleSubmitBooking = async () => {
    ...
    try {
      ...
      // Campaign Images
      if (basicInfo.campaignImages?.length) {
        basicInfo.campaignImages.forEach(file => formData.append('campaignImages', file));
      }
  ...
  return (
    <div className="p-6 w-full text-xs md:ml-64 min-h-screen">
      ...
      <div className="max-w-screen-xl mx-auto">
        ...
        {/* Basic Info */}
        <div className="grid grid-cols-2 gap-6 mb-10">
          {Object.entries(basicInfo).filter(([key]) => key !== 'campaignImages').map(([key, value]) => (
        ...
        {orderInfo.campaigns.filter(c => c.isSaved).map((campaign, cIdx) => {
          ...
          return (
            <div key={cIdx} className="mb-10 border rounded p-4 shadow-sm">
