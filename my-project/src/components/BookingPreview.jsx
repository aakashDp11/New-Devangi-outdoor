...
export default function BookingPreview() {
  ...
  const handleSubmitBooking = async () => {
    ...
    try {
      ...
      console.log("OrderInfo: ", orderInfo.campaigns[0].industry);
  ...
  const handleSaveProposal = async () => {
    ...
    try {
      // --- FIX IS HERE ---
      // The payload now correctly mirrors the structure the backend API expects.
      // 'industry' is explicitly added to the top level.
      const payload = {
        ...basicInfo, // Spreads basic info like clientName, companyName

        // Add industry to the top level, taking it from the first campaign.
        industry: orderInfo.campaigns[0]?.industry || "N/A",

        // Keep the campaigns array as is for detailed information.
        ...
        {orderInfo.campaigns
          ...
          .map((campaign, cIdx) => {
            const campaignTotal = campaign.selectedSpaces.reduce(
              (sum, space) =>
                sum + (space.price || 0) * (space.selectedUnits || 1),
              0
            );
            ...
            return (
              <div
                key={cIdx}
                className="mb-10 w-[90%] border rounded p-4 shadow-sm"
              >
                <h3 className="font-semibold text-lg mb-2 text-blue-700">
                  {campaign.campaignName}
                </h3>
                <div className="grid grid-cols-2 gap-6 mb-4">
                  {console.log("campaign", campaign.industry)}
