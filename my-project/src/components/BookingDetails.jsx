...
// Placeholder components from Code 2 to keep the styling intact
...
const CustomSelect = ({ label, name, value, onChange, options, required = false }) => {
    // Determine the currently selected option object for react-select
    const formattedValue = options.find((option) => option.value === value) || null;
    
    // Custom styles from Code 2, adapted for react-select
    const customStyles = {
...
export default function BookingDetails() {
    ...
    const [isLoading, setIsLoading] = useState(true); // Added loading state from Code 2
    const { isCollapsed } = useSidebar();

    // Industry options (from Code 1/2 - kept as simple array of objects)
    ...
    // --- DATA FETCHING (LOGIC FROM CODE 1) ---
    ...
    useEffect(() => {
        ...
        const fetchSpaces = async () => {
            ...
            // Transformation logic from Code 1
    ...
    useEffect(() => {
        ...
        const fetchBooking = async () => {
            setIsLoading(true); // Set loading start from Code 2
            try {
                const res = await fetch(
                ...
                setIsLoading(false); // Set loading end from Code 2
            }
        };
        fetchBooking();
    }, [id]);

    // --- CAMPAIGN DRAFT HANDLERS (LOGIC FROM CODE 1) ---
    ...
    const saveDraftCampaign = async (index) => {
        ...
        if (!campaign.campaignName || !campaign.industry || !campaign.description || !campaign.startDate || !campaign.endDate) {
            toast.error("Please fill all required campaign fields.");
            return;
        }
    ...
    const handleDeleteCampaign = async (campaignId) => {
        ...
        try {
            // Using the endpoint structure from Code 2's handleDeleteCampaign for robust deletion:
    ...
    // --- LOADING/NO-BOOKING STATE (UI from Code 2) ---
    ...
    // --- CALCULATIONS (LOGIC FROM CODE 1/2) ---
    ...
    // --- MAIN RENDER (UI/STYLES from Code 2) ---
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 w-screen text-base-content">
            ...
            <main className={`h-full overflow-y-auto px-4 sm:px-6 py-6 transition-all duration-300 ${isCollapsed ? "lg:ml-24" : "lg:ml-64"}`}>
                ...
                {/* Client Information & Payment Overview Section */}
                                ...
                                if (key === "bookingSource" && value === "Agency") {
                                    return (
                                        <React.Fragment key={key}>
                                            ...
                                            <InfoDetail label="Agency Name" value={booking.agencyName ?? "NA"} delay={(index + 1) * 100} />
                                        </React.Fragment>
                                    );
                                }
                                return <InfoDetail key={key} label={label} value={value} delay={index * 100} />;
                            })}
                        </div>
                    </div>
                        ...
                        {booking.isFOCBooking ? (
                            ...
                            <div>
                                {totalPaid === 0 && totalDue === 0 && grandTotal === 0 ? (
                                    <div className="flex items-center justify-center h-48">
                                        {/* Updated message based on Code 2 context */}
                                        <p className="text-gray-500 text-md text-center">No payment details available</p>
                                    </div>
                                ) : (
                                    <>
                                        ...
                                        <div className="flex text-xs justify-center mt-2">
                                            <PieChart
                                                series={[
                                                    {
                                                        innerRadius: 45, outerRadius: 70, paddingAngle: 2, cornerRadius: 5,
                                                        data: [
                ...
                {/* New Campaign Drafts Section (UI/Inputs from Code 2) */}
                {campaignDrafts.map((campaign, index) => (
                    ...
                    >
                            ...
                            <div className="col-span-2">
                                ...
                                <div className="flex items-center gap-6">
                                    <div className="flex items-center">
                                        ...
                                        />
                                        <label htmlFor={`foc-yes-${index}`} className="ml-2 text-sm font-medium cursor-pointer">Yes</label>
                                    </div>
                                    <div className="flex items-center">
                                        <input
                                            type="radio"
                                            ...
                                            onChange={(e) => {
                                                const updated = { ...campaign, isFOC: e.target.value === "false", };
                                                updateDraftCampaign(index, updated);
                                            }}
                                            className="h-4 w-4 accent-black cursor-pointer"
                                        />
                                        <label htmlFor={`foc-no-${index}`} className="ml-2 text-sm font-medium cursor-pointer">No</label>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="mt-4">
                            {/* InventorySelector component (assuming it's available as a local import) */}
                            <InventorySelector
                                campaignIndex={index}
                                campaign={campaign}
                                spaces={spaces}
                ...
                {/* Add New Campaign Button (UI/Style from Code 2) */}
                <button
            ...
            {/* Delete Confirmation Modal (UI/Style from Code 2) */}
            {showDeletePopup && (
...
// Retaining simple reusable components from Code 1 to ensure no local function references break,
// even though the main component uses the styled ones from Code 2.

// (These are technically redundant but kept for completeness in case other files import them,
// though they are not used within the main BookingDetails export now).
