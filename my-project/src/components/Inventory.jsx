...
const Button = ({ children, className = '', ...props }) => ( 
  <button 
    className={`px-4 py-2 rounded-md bg-[var(--color-primary)] text-white text-xs font-medium hover:opacity-90 transition ${className}`} 
    {...props}
  >
    {children}
  </button> 
...
const Pagination = ({ currentPage, totalPages, onPageChange, totalCount, itemsPerPage }) => {
  ...
  if (totalCount === 0) return <div className="text-center py-10 text-[var(--color-muted)]">No inventories found.</div>;
  const startItem = (currentPage - 1) * itemsPerPage + 1;
  const endItem = Math.min(currentPage * itemsPerPage, totalCount);
  return (
    <div className="flex flex-col sm:flex-row justify-between items-center mt-8 text-xs gap-4">
       <span className="text-[var(--color-muted)]">Showing {startItem} - {endItem} of {totalCount} results</span>
       {totalPages > 1 && (
         <div className="flex items-center gap-4">
             <button 
               onClick={() => onPageChange(currentPage > 1 ? currentPage - 1 : 1)} 
               className="px-3 py-1.5 rounded-md bg-[var(--color-surface)] border border-[var(--color-border)] hover:bg-[var(--color-hover)] disabled:opacity-50 transition" 
             ...
             <form onSubmit={handlePageSubmit} className="flex items-center gap-2">
               ...
               <input 
                 ...
                 className="w-12 h-8 text-center border border-[var(--color-border)] rounded-md bg-[var(--color-surface)] text-[var(--color-text)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]"
               />
...
const AvailabilityBadge = ({ availabilityStatus }) => {
  ...
  switch (availabilityStatus) {
    ...
    case 'Booked': 
      colorClasses = 'bg-[var(--color-danger-light)] text-[var(--color-danger-text)]'; 
      text = 'Booked'; 
      break;
    case 'Partially available': 
    case 'Partialy available': 
      colorClasses = 'bg-[var(--color-warning-light)] text-[var(--color-warning-text)]'; 
    ...
    case 'Overlapping booking': 
      ...
      break;
    default: 
      colorClasses = 'bg-[var(--color-success-light)] text-[var(--color-success-text)]'; 
      text = 'Available'; 
      break;
  }
...
const SortableHeader = ({ title, sortKey, sortConfig, setSortConfig }) => {
  ...
  return (
    <th scope="col" className="px-6 py-3">
      <div onClick={handleSort} className="flex items-center gap-1.5 cursor-pointer select-none text-[var(--color-text)] hover:text-[var(--color-primary)]">
        {title}
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[9999]" onClick={onClose}>
      <div className="relative p-4 rounded-lg shadow-lg max-w-screen-lg max-h-screen-lg" onClick={(e) => e.stopPropagation()}>
        <button 
          onClick={onClose} 
          className="absolute top-2 right-2 text-white text-2xl bg-[var(--color-surface)] hover:bg-[var(--color-hover)] rounded-full p-1 cursor-pointer transition"
        >
          <FaTimes />
        </button>
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 text-xs gap-5">
      {data.map((item) => {
        ...
        return (
          <div key={item._id} className="bg-[var(--color-surface)] border border-[var(--color-border)] shadow-sm rounded-lg text-xs hover:shadow-md cursor-pointer transition-shadow flex flex-col">
            <img 
              src={item.mainPhoto || 'https://via.placeholder.com/300x200'} 
              alt="Space" 
              className="w-full h-40 object-cover rounded-t-lg bg-[var(--color-muted-light)] cursor-pointer" 
            ...
            <div className="p-4 flex flex-col flex-grow">
              <div className="flex-grow" onClick={() => navigate(`/space/${item._id}`)}>
                <div className="flex justify-between items-start gap-2">
                  <h3 className="font-semibold text-[var(--color-text)] leading-tight">{item.spaceName}</h3>
                ...
                <div className="flex gap-2 text-xs flex-wrap mb-2">
                  <span className="text-xs px-2 py-1 rounded-full bg-[var(--color-primary-light)] text-[var(--color-primary-text)]">{item.city || 'N/A'}</span>
                  <span className="text-xs px-2 py-1 rounded-full bg-[var(--color-secondary-light)] text-[var(--color-secondary-text)]">{item.spaceType || 'N/A'}</span>
                </div>
                <div className="flex gap-1.5 flex-wrap">
                  {tags.map((tag, idx) => (
                    <div key={idx} className="relative group text-xs px-2 py-1 rounded-full bg-[var(--color-muted-light)] text-[var(--color-text)] flex items-center">
                      {tag}
                      <span 
                        onClick={(e) => { e.stopPropagation(); onTagUpdate('remove', item._id, tag); }} 
                        className="ml-1.5 text-[var(--color-danger)] hidden group-hover:inline cursor-pointer font-bold"
                      >
                        ×
                      </span>
                    </div>
              ...
              <div className="mt-4 text-xs">
                <input 
                  placeholder="+ Add Tag" 
                  className="px-2 py-1 w-full border border-[var(--color-border)] rounded-md bg-[var(--color-surface)] text-[var(--color-text)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]" 
                  onKeyDown={(e) => { 
                    e.stopPropagation(); 
  ...
  return (
    <div className="overflow-x-auto bg-[var(--color-surface)] rounded-lg shadow-sm border border-[var(--color-border)]">
      <table className="w-full text-xs text-left text-[var(--color-muted)]">
        ...
        <tbody>
          {data.map((item, index) => (
            <tr key={item._id} className="bg-[var(--color-surface)] border-b border-[var(--color-border)] hover:bg-[var(--color-hover)]">
              <td className="px-6 py-4 text-[var(--color-muted)] cursor-pointer" onClick={() => navigate(`/space/${item._id}`)}>
              ...
              <td className="px-6 py-4 font-medium text-[var(--color-text)] whitespace-nowrap">
                <div className="flex items-center gap-3">
                  <img 
                    ...
                    alt={item.spaceName} 
                    className="w-10 h-10 object-cover rounded-md bg-[var(--color-muted-light)] cursor-pointer hover:opacity-80 transition-opacity" 
                    onClick={(e) => { 
                      e.stopPropagation(); 
                  ...
                  <div className="cursor-pointer" onClick={() => navigate(`/space/${item._id}`)}>
                    <div className="font-semibold text-[var(--color-text)]">{item.spaceName}</div>
                    <div className="text-[var(--color-muted)] text-xs">{item.address}</div>
                  </div>
                </div>
              </td>
              <td className="px-6 py-4 cursor-pointer text-[var(--color-text)]" onClick={() => navigate(`/space/${item._id}`)}>{item.city}</td>
              <td className="px-6 py-4 cursor-pointer text-[var(--color-text)]" onClick={() => navigate(`/space/${item._id}`)}>{item.spaceType}</td>
              ...
              <td className="px-6 py-4 cursor-pointer text-[var(--color-text)]" onClick={() => navigate(`/space/${item._id}`)}>{item.ownershipType}</td>
              <td className="px-6 py-4 font-mono text-xs text-[var(--color-muted)] cursor-pointer" onClick={() => navigate(`/space/${item._id}`)}>{item.inventoryId || item._id.slice(-8).toUpperCase()}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
const InventoryMapView = ({ spaces, navigate }) => {
    ...
    return <div ref={mapContainerRef} className="w-full h-[65vh] rounded-lg border border-[var(--color-border)] shadow-sm" />;
};
...
const handleConfirmUpload = async () => {
 ...
 const handleApplyDateFilter = () => {
  ...
  return (
    <div className="min-h-screen bg-[var(--color-background)] w-screen text-[var(--color-text)] flex flex-col lg:flex-row">
      <Navbar />
      <main className={`flex-1 h-screen overflow-y-auto px-4 md:px-6 py-8 transition-all duration-300 ${isCollapsed ? 'lg:ml-24' : 'lg:ml-64'}`}>
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-2 text-xs">
            ...
            <label htmlFor="excel-upload" className="cursor-pointer px-4 py-2 rounded-md bg-[var(--color-primary)] text-white text-xs font-medium hover:opacity-90 transition">Upload Excel</label>
            <Button onClick={handleDownloadExcel}>Download Excel</Button>
          </div>
        </div>

        <div className="mt-6 p-4 bg-[var(--color-surface)] rounded-lg shadow-sm border border-[var(--color-border)]">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <input 
              ...
              className="w-full md:w-[50%] px-4 py-2 text-xs border border-[var(--color-border)] rounded-md bg-[var(--color-surface)] text-[var(--color-text)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]" 
            />
            <div className="flex items-center gap-2 flex-shrink-0">
              <div className="flex items-center p-1 bg-[var(--color-muted-light)] rounded-lg border border-[var(--color-border)]">
                {[
                  ...
                  <button key={v.mode} onClick={() => setViewMode(v.mode)} className={`p-1.5 rounded-md transition ${viewMode === v.mode ? 'bg-[var(--color-surface)] shadow text-[var(--color-text)]' : 'text-[var(--color-muted)] hover:bg-[var(--color-hover)]'}`}>
                    {v.icon}
                  </button>
                ))}
              </div>
              <button onClick={resetFilters} className="px-4 py-2 border border-[var(--color-border)] rounded-md text-xs bg-[var(--color-surface)] text-[var(--color-text)] hover:bg-[var(--color-hover)] transition">Reset Filters</button>
            </div>
          </div>
          <div className="mt-4 flex flex-wrap gap-3 text-xs items-center">
            <input 
              className="px-3 py-2 border border-[var(--color-border)] rounded-md w-full md:w-auto bg-[var(--color-surface)] text-[var(--color-text)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]" 
            ...
            <div ref={datePickerRef} className="relative w-full md:w-auto">
                <button 
                  ...
                  className="px-4 py-2 border border-[var(--color-border)] rounded-md hover:bg-[var(--color-hover)] w-full text-left bg-[var(--color-surface)] text-[var(--color-text)] transition"
                >
                    {dateRange[0].startDate && dateRange[0].endDate ? `${formatDate(dateRange[0].startDate)} to ${formatDate(dateRange[0].endDate)}` : "Date Filter"}
                </button>
                {showDateModal && (
                    <div className="absolute top-full mt-2 left-0 bg-[var(--color-surface)] rounded-xl shadow-lg p-2 border border-[var(--color-border)] z-50 text-xs" onClick={(e) => e.stopPropagation()}>
                        <DateRange
                        ...
                        <div className="flex justify-end gap-2 p-2 border-t border-[var(--color-border)]">
                            <button 
                              ...
                              className="px-4 py-1.5 rounded-md bg-[var(--color-muted-light)] text-[var(--color-text)] hover:bg-[var(--color-hover)] transition"
                            >
                              Cancel
                            </button>
                            <button 
                              onClick={handleApplyDateFilter} 
                              className="px-4 py-1.5 rounded-md bg-[var(--color-primary)] text-white hover:opacity-90 transition"
                            >
                              Apply
                            </button>
                        </div>
        ...
        {showUploadModal && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            <div className="bg-[var(--color-surface)] rounded-xl shadow-lg p-6 w-96 border border-[var(--color-border)]">
              <h2 className="text-lg font-semibold mb-4 text-[var(--color-text)]">Upload Inventory Excel</h2>
              <p className="mb-4 text-sm text-[var(--color-muted)]">Selected File: <span className="font-medium text-[var(--color-text)]">{selectedFile?.name}</span></p>
              <div className="flex justify-end gap-2">
                <button 
                  onClick={() => { setShowUploadModal(false); setSelectedFile(null); }} 
                  className="px-4 py-2 rounded-md bg-[var(--color-muted-light)] text-[var(--color-text)] hover:bg-[var(--color-hover)] transition"
                >
                  Cancel
                </button>
                <button 
