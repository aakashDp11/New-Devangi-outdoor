...
// Leaflet default icon setup from Code 1
...
// Updated modern Tooltip component
const Tooltip = ({ content, children }) => {
    ...
    return (
        <div className="relative flex items-center" onMouseEnter={() => setIsVisible(true)} onMouseLeave={() => setIsVisible(false)}>
            {children}
            {isVisible && (
                <div className="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 px-3 py-2 rounded-xl bg-gray-800 text-white text-xs whitespace-nowrap opacity-95 animate-fadeIn pointer-events-none shadow-lg backdrop-blur-sm border border-gray-700/20">
                    {content}
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[9999] animate-fadeIn" onClick={onClose}>
            <div className="relative p-4 rounded-lg shadow-lg max-w-screen-lg max-h-screen-lg animate-scaleIn" onClick={(e) => e.stopPropagation()}>
                <button
                    onClick={onClose}
                    className="absolute top-2 right-2 text-white text-2xl bg-black/50 hover:bg-black/70 rounded-full p-2 cursor-pointer transition-all duration-200 hover:scale-110 hover:rotate-90"
                >
                    <FaTimes />
                </button>
                <img
...
// Card component with borderless design (Retained for other uses like Map/Filters)
...
const Pagination = ({ currentPage, totalPages, onPageChange, totalCount, itemsPerPage }) => {
    ...
    if (totalCount === 0) return null; // No need to show pagination if totalCount is 0, handled by main component
...
// AvailabilityBadge Component (Unchanged)
...
// SortableHeader Component (Updated padding to py-4)
...
const CustomSelect = ({ value, onChange, options, placeholder, className = '' }) => {
    return (
        <div className={`relative w-full md:w-auto ${className}`}>
            <select
                className="px-3 py-2 rounded-xl w-full bg-white text-black focus:outline-none focus:ring-2 focus:ring-black transition-all duration-200 shadow-sm hover:shadow-md appearance-none pr-8 text-xs"
                value={value}
                onChange={onChange}
            >
                        ...
                        <div className="p-4 flex flex-col flex-grow">
                            <div className="flex-grow" onClick={() => navigate(`/space/${item._id}`)}>
                                <div className="flex justify-between items-start gap-2">
                                    <h3 className="font-semibold text-black leading-tight hover:text-gray-700 transition-colors duration-200">
                                        {item.spaceName}
                                    </h3>
                                ...
                                <div className="flex gap-2 text-xs flex-wrap mb-2">
                                    <span className={`text-xs px-2 py-1 rounded-full bg-gray-200 text-gray-800 transition-all duration-200 hover:scale-105`}>
                                        {item.spaceType || 'N/A'}
                                    </span>
                                    <span className={`text-xs px-2 py-1 rounded-full bg-gray-200 text-gray-800 transition-all duration-200 hover:scale-105`}>
                                ...
                                <div className="flex gap-1.5 flex-wrap">
                                    {tags.map((tag, idx) => (
                                        <div
                                            key={idx}
                                            className="relative group text-xs px-2 py-1 rounded-full bg-gray-100 text-black flex items-center transition-all duration-200 hover:bg-gray-200 hover:scale-105 animate-slideIn"
                            ...
                            <div className="mt-4 text-xs">
                                <input
                                    placeholder="+ Add Tag"
                                    className={`px-2 py-1 w-full rounded-lg bg-gray-50 text-black focus:outline-none focus:ring-2 transition-all duration-200 shadow-sm hover:shadow-md ${
                                        tagInputErrors[item._id] ? 'ring-red-300' : 'ring-black'
                                    }`}
                                    onKeyDown={(e) => {
                                        e.stopPropagation();
...
const InventoryTableView = ({ data, currentPage, limit, navigate, sortConfig, setSortConfig, onImageClick }) => {
    ...
    return (
        // Replaced Card with a plain div and applied styling directly for a clean table look
                ...
                <table className="w-full text-xs text-left text-gray-600">
                    <thead className="text-xs text-black uppercase bg-gray-100 sticky top-0 z-10 border-b border-gray-300">
                        <tr className="border-b-2 border-gray-200">
                            {/* Apply rounding to the first and last header cells for the top corners */}
                            ...
                            <SortableHeader title="Inventory ID" sortKey="inventoryId" sortConfig={sortConfig} setSortConfig={setSortConfig} className="rounded-tr-xl" />
                        </tr>
                    </thead>
                    <tbody>
                        {data.map((item, index) => (
                            ...
                            >
                                ...
                                <td className="px-6 py-4 font-medium text-black whitespace-nowrap">
                                    <div className="flex items-center gap-3">
                                        <div className="relative overflow-hidden rounded-md shadow flex-shrink-0">
                                            {item.mainPhoto ? (
                                                <img
                                                    src={item.mainPhoto}
                                                    alt={item.spaceName}
                                        ...
                                        <div className="transition-all duration-200 hover:translate-x-1">
                                            <div className="font-semibold text-black hover:text-gray-700 transition-colors duration-200">
                                                {item.spaceName}
                                            </div>
                                ...
                                <td className="px-6 py-4 text-black hover:text-gray-700 transition-all duration-200">{item.city}</td>
                                <td className="px-6 py-4 text-black">{item.spaceType}</td>
                                <td className="px-6 py-4 transition-all duration-200">
                                    <AvailabilityBadge availabilityStatus={item.availability} />
                                </td>
                                <td className="px-6 py-4 text-black">{item.ownershipType}</td>
...
export default function InventoryDashboard() {
    ...
    }, [search, selectedRegion, availability, startDate, endDate, spaceType, ownershipType, navigate, addNotification]);


    // Effect to call the correct fetch function based on viewMode and filters
    useEffect(() => {
        if (viewMode === 'map') {
            fetchMapLocations();
        } else {
        ...
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 w-screen text-black flex flex-col lg:flex-row">
            ...
            <main className={`flex-1 h-screen overflow-y-auto px-4 md:px-6 py-8 transition-all duration-300 ${isCollapsed ? 'lg:ml-24' : 'lg:ml-64'}`}>
                <div className='flex flex-col md:flex-row justify-between items-start md:items-center mb-6 animate-slideDown'>
                    ...
                    <div className="flex items-center gap-2 text-xs mt-4 md:mt-0">
                        ...
                        <input
                            ...
                            onClick={(e) => { e.target.value = null; setFileError(''); }} // Clear input on click
                            className="hidden"
                        />
                        <label
                            htmlFor="excel-upload"
                            className="cursor-pointer px-4 py-2 rounded-xl bg-black text-white text-xs font-medium hover:opacity-90 transition-all duration-200 hover:scale-105 active:scale-95 shadow-md hover:shadow-lg"
                        >
                            Upload Excel
                        </label>
                ...
                <Card className="mt-6 shadow-xl animate-slideUp bg-gray-100 bg-opacity-80">
                    <CardContent className="p-0">
                        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                            <div className="w-full md:w-[50%]">
                                <input
                                    ...
                                    className={`w-full px-4 py-2 text-xs rounded-xl bg-white text-black focus:outline-none focus:ring-2 transition-all duration-200 shadow-sm hover:shadow-md ${
                                        searchError ? 'ring-red-300' : 'ring-black'
                                    }`}
                                />
                            ...
                            <div className="flex items-center gap-3 flex-shrink-0">
                                <div className="flex items-center p-1 bg-gray-200 rounded-xl shadow-inner gap-2">
                                    {[
                                        ...
                                        <Tooltip key={v.mode} content={`${v.mode.charAt(0).toUpperCase() + v.mode.slice(1)} View`}>
                                            <button
                                                onClick={() => setViewMode(v.mode)}
                                                className={`p-2 rounded-xl transition-all duration-200 hover:scale-105 shadow-md hover:shadow-lg ${
                                                    viewMode === v.mode
                                                        ? 'bg-gray-100 text-black'
                                                        : 'bg-white text-gray-500 hover:bg-gray-300 hover:text-gray-700'
                                                }`}
                                            >
                                                {v.icon}
                                            </button>
                        ...
                        <div className="mt-4 flex flex-wrap gap-3 text-xs items-center animate-slideUp" style={{ animationDelay: '200ms' }}>
                            <input
                                className="px-3 py-2 rounded-xl w-full md:w-auto bg-white text-black focus:outline-none focus:ring-2 focus:ring-black transition-all duration-200 shadow-sm hover:shadow-md"
                ...
                <div className="mt-6 relative">
                    {loading && (
                        <div className="absolute inset-0 bg-white/80 rounded-lg flex items-center justify-center z-10">
                            <div className="flex flex-col items-center gap-3">
                                <div className="w-8 h-8 border-2 border-black border-t-transparent rounded-full animate-spin"></div>
                                <div className="text-gray-500 text-sm">Loading inventories...</div>
                            </div>
                        </div>
                    )}

                    {/* InventoryTableView is now a standalone, structured table */}
                    {viewMode === 'table' && totalCount > 0 && <InventoryTableView data={sortedSpaces} currentPage={currentPage} limit={limit} navigate={navigate} sortConfig={sortConfig} setSortConfig={setSortConfig} onImageClick={setPreviewImageUrl} />}
                    {viewMode === 'grid' && totalCount > 0 && <InventoryGridView data={spaces} onTagUpdate={handleTagUpdate} navigate={navigate} onImageClick={setPreviewImageUrl} />}
                    {viewMode === 'map' && totalCount > 0 && <InventoryMapView spaces={mapSpaces} navigate={navigate} />}
                    
                    {!loading && totalCount === 0 && (
                        <div className="text-center py-20 animate-fadeIn bg-white rounded-xl shadow-xl">
                            <div className="text-gray-500 text-lg mb-2">📦</div>
                            <div className="text-gray-500">No inventories found matching your filters.</div>
                        </div>
                    )}
                </div>
                ...
                {showUploadModal && (
                    <Modal onClose={() => { setShowUploadModal(false); setSelectedFile(null); setFileError(''); }}>
                        <div className="p-6">
                            <h2 className="text-lg font-semibold mb-4 text-black">Upload Inventory Excel</h2>
                            <div className="mb-4">
                                <p className="mb-2 text-sm text-gray-500">
                                    Selected File: <span className="font-medium text-black">{selectedFile?.name}</span>
                                </p>
                            ...
                            <div className="flex justify-end gap-2">
                                <button
                                    onClick={() => { setShowUploadModal(false); setSelectedFile(null); setFileError(''); }}
                                    className="px-4 py-2 rounded-xl bg-gray-100 text-black hover:bg-gray-200 transition-all duration-200 transform hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none shadow-md"
            ...
            {/* Tailwind Keyframes/Animation Styles (Unchanged) */}
            <style jsx>{`
