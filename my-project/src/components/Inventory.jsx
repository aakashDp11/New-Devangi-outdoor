...
const SortableHeader = ({ title, sortKey, sortConfig, setSortConfig }) => {
  ...
  return (
    <th scope="col" className="px-6 py-3">
      ...
      >
        {title}
        <span className={`text-[var(--color-muted)] transition-all duration-200 ${isSorting ? 'text-black scale-110' : ''}`}>
            ...
            <div className="p-4 flex flex-col flex-grow">
              <div className="flex-grow" onClick={() => navigate(`/space/${item._id}`)}>
                <div className="flex justify-between items-start gap-2">
                  <h3 className="font-semibold text-[var(--color-text)] leading-tight hover:text-black transition-colors duration-200">
                    {item.spaceName}
                  </h3>
          ...
          <tbody>
            {data.map((item, index) => (
              ...
              >
                ...
                <td className="px-6 py-4 cursor-pointer text-[var(--color-text)] hover:text-black transition-all duration-200"
                  onClick={() => navigate(`/space/${item._id}`)}>{item.city}</td>
                <td className="px-6 py-4 cursor-pointer text-[var(--color-text)]"
                  onClick={() => navigate(`/space/${item._id}`)}>
                  {item.spaceType}
                </td>
                ...
                <td className="px-6 py-4 font-mono text-xs text-[var(--color-muted)] cursor-pointer hover:text-black transition-all duration-200"
                  onClick={() => navigate(`/space/${item._id}`)}>
...
export default function InventoryDashboard() {
  ...
  // Load viewMode from localStorage on initial render
  useEffect(() => {
