...
// /* ---------- Subdocuments (existing omitted for brevity) ---------- */
// const BookingStatusSchema = new Schema({
//   confirmed: { type: Boolean, default: false },
//   reference: { type: String },
//   bookingDate: { type: String },         // keep as String if that’s what your UI sends
//   estimateDocument: { type: String },     // S3 URL
//   completedAt: { type: Date }
// }, { _id: false, timestamps: true });
...
//   // same semantics as Space DigitalStatusSchema, but campaign-scoped
//   confirmed: { type: Boolean, default: false },
...
//   // Per-unit digital status lives here
//   units: {
...
//         return (
//           ids.length === set.size &&
//           ids.every(n => Number.isInteger(n) && n >= 1)
//         );
//       },
//       message: 'units[].unitId must be unique positive integers.'
//     }
//   }
// }, { _id: false });
...
//   // keep legacy flattened list if you want
//   spaces: [{ type: Schema.Types.ObjectId, ref: 'Space' }],
//     invoice: [InvoiceItemSchema],
//   cashMemo: [CashMemoItemSchema],
//   creditNote: [CreditNoteItemSchema],

//   // NEW: allocations with per-unit digital status for DOOH
//   allocations: {
...
//   artwork: { type: ArtworkSchema, default: () => ({}) },
//   po: { type: PoSchema, default: () => ({}) },
//   invoice: { type: InvoiceItemSchema, default: () => ({}) },
//   payment: { type: PaymentItemSchema, default: () => ({}) },


//   // ... (artwork, bookingStatus, po, invoice, payment — unchanged)
// }, { timestamps: true });
...
// pipelineSchema.pre('save', function(next) {
//   // existing timestamp logic for booking/po/artwork ...

//   // keep legacy `spaces` flattened (optional)
//   if (this.isModified('allocations')) {
//     const ids = (this.allocations || []).map(a => a.space).filter(Boolean);
//     this.spaces = Array.from(new Set([...(this.spaces || []), ...ids].map(String)))
...
//   const idx = alloc.units.findIndex(u => u.unitId === Number(unitId));
//   if (idx === -1) {
