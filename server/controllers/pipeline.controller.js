...
export const getPipelineByCampaignId = async (req, res) => {
  ...
  try {
    ...
    const mappings = await CampaignInventoryMapping.find({ campaignId }, {
      ...
      startDate: 1, endDate: 1, unitIds: 1,printingStatus:1,mountingStatus:1,
      digitalStatus: 1,
      createdAt: 1, updatedAt: 1
    }).lean();
 console.log("CampaignInv Mappings are",mappings);
 const spaceIds = [...new Set(mappings.map(m => String(m.spaceId)))].map(id => new mongoose.Types.ObjectId(id));
 console.log("Corresponding space ids are",spaceIds);
 ...
 console.log("Space map is",spaceMap);
    ...
    const spacesResponse = mappings.map(m => {
      ...
      return {
        ...
        // },
        printingStatus: m?.printingStatus ,
        mountingStatus: m?.mountingStatus,
