...
router.get('/map-locations', authenticate, async (req, res) => {
  try {
    ...
    // ----- date window (defaults to "today" if not provided) -----
    const checkStartDate = startDate ? new Date(startDate) : new Date();
    const checkEndDate   = endDate   ? new Date(endDate + 'T23:59:59.999Z') : new Date();
    ...
    const pipeline = [
      ...
      // Join overlapping mappings (spaceId + date overlap)
      {
        $lookup: {
          from: 'campaigninventorymappings',
          let: { spaceId: '$_id', s: checkStartDate, e: checkEndDate },
          pipeline: [
            {
              $match: {
                $expr: {
                  $and: [
                    { $eq: ['$spaceId', '$$spaceId'] },
                    { $lte: [ { $toDate: '$startDate' }, '$$e' ] },
                    { $gte: [ { $toDate: '$endDate'   }, '$$s' ] }
                  ]
                }
              }
            },
            // count units booked by this mapping in the window
            { $project: { unitCount: { $size: { $ifNull: ['$unitIds', []] } } } }
          ],
          as: 'overlaps'
        }
      },

      // Sum booked units across overlaps
      {
        $addFields: {
          unitsBookedInPeriod: {
            $ifNull: [ { $sum: '$overlaps.unitCount' }, 0 ]
          }
        }
      },
              ...
              in: {
                $switch: {
                  branches: [
                    ...
                    // if a window was provided and there is some booking, label as overlapping
                    { case: { $gt: ['$$booked', 0] }, then: startDate ? 'Overlapping booking' : 'Partially available' }
                  ],
      ...
      // Optional filter by computed availability
      ...(availability ? [{ $match: { availability } }] : []),

      // Minimal payload for map markers
      {
        $project: {
          spaceName: 1,
          latitude: 1,
...
//   }
// });
// Update Digital Status (DOOH only)
// Supports:
//   PUT /api/spaces/:id/digital-status/:unitId
//   PUT /api/spaces/:id/digital-status        (with { unitId } in body)
router.put('/:id/digital-status/:unitId', async (req, res) => {
  try {
    const { id, unitId: unitIdParam } = req.params;
    const {
      unitId: unitIdBody,
      confirmed,
      isLive,
    ...
    const unitIdRaw = unitIdParam ?? unitIdBody;
    const unitId = Number(unitIdRaw);

    const space = await Space.findById(id);
    if (!space) return res.status(404).json({ error: 'Space not found' });

    // Digital status applies ONLY to DOOH as per new rules
    if (space.spaceType !== 'DOOH') {
      return res.status(400).json({ error: 'Digital status is only available for DOOH spaces.' });
    }

    // Validate unitId
    if (!Number.isInteger(unitId) || unitId < 1) {
      return res.status(400).json({ error: 'A valid unitId (positive integer) is required.' });
    }
    const totalUnits = Math.max(1, Number(space.unit || 1));
    if (unitId > totalUnits) {
      return res.status(400).json({ error: `unitId exceeds available units. Allowed range: 1..${totalUnits}` });
    }

    // Ensure digitalStatus array exists and has the right slot for unitId
    if (!Array.isArray(space.digitalStatus)) {
      space.digitalStatus = [];
    }

    let idx = space.digitalStatus.findIndex(ds => Number(ds?.unitId) === unitId);
    if (idx === -1) {
      // Create a fresh entry for this unitId
      space.digitalStatus.push({ unitId });
      idx = space.digitalStatus.length - 1;
    }

    const target = space.digitalStatus[idx];

    // Update only provided fields
    if (typeof confirmed !== 'undefined') target.confirmed = !!confirmed;
    if (typeof isLive !== 'undefined') target.isLive = !!isLive;
    if (typeof goLiveDate !== 'undefined') target.goLiveDate = goLiveDate; // keep as string per schema
    if (typeof assignedPerson !== 'undefined') target.assignedPerson = assignedPerson;
    if (typeof assignedAgency !== 'undefined') target.assignedAgency = assignedAgency;
    if (typeof note !== 'undefined') target.note = note;

    // Let schema hooks handle completedAt/liveCompletedAt
    space.markModified('digitalStatus');

    await space.save();

    // Return only the updated item + meta to keep response light
    return res.json({
      spaceId: space._id,
      unitId,
      digitalStatus: target,
      totalUnits
    });
  } catch (error) {
    console.error('Error updating digital status:', error);
